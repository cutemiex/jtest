...
public class CollaborativeTransferQueue<E> extends AbstractQueue<E> implements
    ...
    static class TransferQueue extends Transferer {
        static final class QNode {
            ...
            /**
             * Returns true if this node is known to be off the queue because its next pointer has been forgotten due to
        ...
        void clean(QNode pred, QNode s) {
            ...
            /*
             * At any given time, exactly one node on list cannot be deleted -- the last inserted node. To accommodate
             * this, if we cannot delete s, we save its predecessor as "cleanMe", deleting the previously saved version
             * first. At least one of node s or the node previously saved can always be deleted, so this always
             * terminates.
             */
    ...
    /**
     * Inserts the specified element into this queue, waiting if necessary up to the specified wait time for another
     * thread to receive it.
     * 
     * @return <tt>true</tt> if successful, or <tt>false</tt> if the specified waiting time elapses before a consumer
     *         appears.
     * @throws InterruptedException {@inheritDoc}
     * @throws NullPointerException {@inheritDoc}
     */
    ...
    /**
     * Returns <tt>false</tt> unless the given collection is empty. A <tt>SynchronousQueue</tt> has no internal
     * capacity.
     * 
    ...
    /**
     * Sets the zeroeth element of the specified array to <tt>null</tt> (if the array has non-zero length) and returns
     * it.
     * 
     * @param a the array
    ...
    /*
     * To cope with serialization strategy in the 1.5 version of SynchronousQueue, we declare some unused classes and
     * fields that exist solely to enable serializability across versions. These fields are never used, so are
